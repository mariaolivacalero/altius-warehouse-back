...
class ReceptionBatchTest(TestCase):
    ...
    def test_create_reception_batch(self):
        ...
        created_reception_batch = ReceptionBatch.objects.get(
            receiptID=new_reception_batch_data["receiptID"]
        )
        self.assertEqual(
            created_reception_batch.receiptID, new_reception_batch_data["receiptID"]
        )

    def test_create_reception_batch_bad_request(self):
        url = reverse("reception-batches")
        new_reception_batch_data = {}
        response = self.client.post(url, new_reception_batch_data, format="json")
    ...
    def test_get_single_reception_batch_not_found(self):
        url = reverse("reception-batch-detail", args=["1234"])
        response = self.client.get(url)
    ...
    def test_update_reception_batch(self):
        ...
        self.assertEqual(
            updated_reception_batch.total_quantity, updated_data["total_quantity"]
        )
        self.assertEqual(updated_reception_batch.notes, updated_data["notes"])

    def test_update_reception_batch_bad_request(self):
        url = reverse("reception-batch-detail", args=[self.reception_batch.id])
        updated_data = {}
        response = self.client.put(url, updated_data, format="json")
        self.assertEqual(response.status_code, 400)
